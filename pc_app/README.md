...
Post-move correction is not continuous closed-loop control. It only runs after eligible manual joint endpoint moves, Go Home, or the explicit **Align Shoulder** button, within configured limits. `deadband_deg` is the small-error zone where no correction is attempted; `max_delta_deg` is the hard safety cap where correction is refused. `fault_tolerance_deg` controls warning/fault behavior only; raising it does not increase the correction movement range. Go Home remains a planned move to the configured home pose followed by optional shoulder verification/correction; a single shoulder encoder does not auto-home the robot or make the full TCP pose known. **Align Shoulder** is idle-only and nudges only the shoulder toward the current planning/target shoulder angle.
