...
@pytest.mark.parametrize(
    ...
    [
        ([0.0, 25.0, 80.0, -50.0], [40.0, 0.0, 0.0]),
        ([0.0, 25.0, 80.0, -50.0], [-40.0, 0.0, 0.0]),
...
def test_cartesian_jog_simulation_keeps_reachable_xyz_directions_straight(
    ...
    assert result["metrics"]["alignment"] > 0.999
    assert result["metrics"]["max_lateral_mm"] < 0.05
    assert result["metrics"]["backward_steps"] == 0
...
def test_cartesian_jog_simulation_blocks_bad_local_direction():
    ...
    assert any("direction is unavailable" in note for note in result["notes"])
